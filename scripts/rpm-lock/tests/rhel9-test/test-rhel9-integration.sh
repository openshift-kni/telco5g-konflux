...
# Integration test for RHEL 9 RPM lock generation
# This test validates the full end-to-end functionality of the generate-rhel9-locks.sh script
...
TEST_DIR="$SCRIPT_DIR"
RHEL9_SCRIPT="${PARENT_DIR}/generate-rhel9-locks.sh"
...
echo "=== RHEL 9 RPM Lock Generation Integration Test ==="
echo

# Test 1: Check prerequisites
echo "Test 1: Check prerequisites"
if [[ ! -f "$RHEL9_SCRIPT" ]]; then
    print_test_result "RHEL 9 script exists" "FAIL" "Script not found at $RHEL9_SCRIPT"
    exit 1
else
    print_test_result "RHEL 9 script exists" "PASS"
fi
...
if [[ "$PODMAN_AVAILABLE" == "true" ]]; then
    ...
    print_info "Executing: $RHEL9_SCRIPT $TEST_DIR"

    # Capture output and exit code
    if OUTPUT=$("$RHEL9_SCRIPT" "$TEST_DIR" 2>&1); then
        SCRIPT_EXIT_CODE=0
    else
        SCRIPT_EXIT_CODE=$?
