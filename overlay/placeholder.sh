...
# Function to read YAML variables and assign to local bash variables
read_yaml_variables() {
    local yaml_file="$1"
    ...
    for var in "${variables[@]}"; do
        if value=$(yq e ".$var" "$yaml_file" 2>/dev/null) && [[ "$value" != "null" ]]; then
            echo "Read $var: $value"
            declare "$var=$value"
        else
    ...
    # At this point, all variables are available within this function
    # If you need to use them outside this function, you'll need to either:
    # 1. Return them in some way, or
    # 2. Process them within this function
}

# Usage example:
# read_yaml_variables "variables.yaml"
